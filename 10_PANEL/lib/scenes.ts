 ...
 *
 * Pure: runs in the browser and in unit tests.
 */
...
const FULL_RX = /SHM-EP(\d{3})-SC(\d{3})-SH(\d{4})/i
...
export const shotId = (episode: string, scene: number) => `SHM-${episode}-SC${pad(scene, 3)}-SH0010`

/** Highest scene number used per episode, from shot ids (queued jobs, files on disk). */
function lastScene(shots: string[], episode: string): number {
  let max = 0
  for (const s of shots) {
    const m = s.match(/^SHM-(EP\d{3})-SC(\d{3})/)
...
export function previewScenes(
  ...
  for (const r of rows) {
    ...
    const n = (last.get(r.episode) ?? lastScene(used, r.episode)) + 1
    last.set(r.episode, n)
    out.set(r.key, shotId(r.episode, n))
  }
  return out
}

/** The episode new footage most likely belongs to: the highest one with any shot. */
export function latestEpisode(shots: string[]): string {
  const eps = shots.map((s) => s.match(/^SHM-(EP\d{3})/)?.[1]).filter((e): e is string => !!e).sort()
 ...
 */
export function sceneFromDocument(where: { file?: string | null; label?: string | null; prompt?: string | null }, episode: string): string | null {
  for (const s of [where.label, where.file, where.prompt]) {
    const m = String(s ?? '').match(FULL_RX)
    if (m) return `SHM-EP${m[1]}-SC${m[2]}-SH${m[3]}`
  }
  for (const s of [where.label, where.file]) {
    const t = String(s ?? '')
    const m = t.match(EP_SC_RX)
    if (m) return shotId(`EP${pad(m[1], 3)}`, Number(m[2]))
    const sc = t.match(SC_RX)
    if (sc) return shotId(episode, Number(sc[1]))
  }
  return null
}
