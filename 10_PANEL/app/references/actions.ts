...
export async function requestIndexOp(formData: FormData): Promise<OpRequestResult> {
  ...
  try {
    switch (type) {
      ...
      case 'add': {
        const uploads = await readUploads(formData, id)
  ...
  try {
    if (files.length) {
      ...
      for (const f of files) await fs.writeFile(path.join(P.root, f.rel), f.bytes)
    }
    await fs.mkdir(path.dirname(P.indexOps), { recursive: true })
    await fs.appendFile(P.indexOps, JSON.stringify(record) + '\n', 'utf8')
  } catch (e) {
