              ...
              <Card key={b.batchId} className="space-y-4 p-5">
                ...
                <div className="flex flex-wrap items-center gap-3 border-t border-edge pt-4">
                  ...
                  <span className="ml-auto flex items-center gap-2">
                    {b.status === 'queued' && (
                      <Link href="/queue" className="focus-ring rounded text-xs text-muted underline hover:text-fg">Open the queue</Link>
                    )}
                    ...
                    {b.status === 'priced' && (
                      <Button type="button" size="sm" tone="good" pending={busy === b.batchId} pendingLabel="Approving" onClick={() => act(b, () => approveBatch(b.batchId, b.total))}>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button type="button" tone="bad" onClick={async () => { const b = confirm!; setConfirm(null); await act(b, () => discardBatch(b.batchId)) }}>Discard</Button>
          </>
        }
      >
