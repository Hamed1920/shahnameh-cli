...
import { SHOT_RX, checkRow, isVideoModel, toJobInput, type DraftRow, type RowConfig } from '@/lib/batch-rules'
...
export async function generateFromPrompt(formData: FormData): Promise<{ ok: boolean; batchId?: string; error?: string }> {
  ...
  const [queue, catalog, cfg] = await Promise.all([getQueue(), getCatalog(), getWorkerConfig()])
  ...
  try {
    ...
    if (!SHOT_RX.test(target) && !catalog.some((e) => e.id === target)) {
    ...
    for (const token of refs) {
      if (!token.startsWith('@') || !(await resolveRefToken(token))) throw new Invalid(`Reference ${token} does not resolve to a file in the index.`)
    }
