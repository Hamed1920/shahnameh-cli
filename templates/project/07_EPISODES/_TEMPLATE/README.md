...
Copy this folder to `07_EPISODES/{{CODE}}-EP0NN-<TITLE-SLUG>/` when an episode starts.

```
{{CODE}}-EP001-<TITLE-SLUG>/
  SHOTLIST.csv          one row per shot — see columns below
  script/               script, beat sheet, narration
  boards/               storyboards / previz frames
  shots/                {{CODE}}-EP001-SC014-SH0030_V01_T01.mp4 etc.
...
- `shot_id` — `{{CODE}}-EP001-SC014-SH0030`. Count shots in **tens** so you can insert later.
