<#
.SYNOPSIS
  Ingests SHM-JOB blocks authored in Claude Chat or Claude Cowork.
...
.PARAMETER Path
  A job file, or a directory of them. Defaults to 00_PROJECT/sync/inbox.
...
if (-not $Text) {
    ...
    foreach ($f in $files) {
        $sources += @{ Name = $f.Name; Text = (Get-Content -LiteralPath $f.FullName -Raw); File = $f }
    }
}
...
foreach ($entry in $jobs) {
    ...
    if ($target -cmatch '^NEW/([A-Z]{2,3})/([A-Z0-9][A-Z0-9-]*)$') {
        ...
        if (-not $FOLDER_FOR.ContainsKey($kind)) { Fail "NEW proposal uses unknown kind '$kind'"; continue }
        ...
        $ent = Resolve-ShmEntity -Ref $target -Entities @($entities + $newRows)
    ...
    foreach ($tok in (Split-ShmList $h['refs'])) {
        $r = Resolve-ShmRef -Token $tok -Entities $entities -Manifest $manifest -Root $Root
